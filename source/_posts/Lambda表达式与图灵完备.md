...
IF(True, f,x) -> f
同时认为
Fst(f,x) -> f 等同于Pair(True,f,x)
Snd(f,x) -> x 等同于Pair(False,f,x)
这其实就是投影操作。
```
...
我们将0判断、False判断、null判断认为是等价的。文章开始也说了，等价判断是先决条件。所以0判断一定是预先存在的。所以lambda表达式图灵完备了。
