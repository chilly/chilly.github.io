...
```
Design methods like *put, get, containsKey, containsValue, remove* etc and answer the *complexity*.
```

All of us knows "HashMap" means. It is not "HashTable" see **Figure 1.1**. It means there does not exist a big array to store all of data.
...
T(n) = O(hash) + O(look up space) + T(n-1)
So complexity is no more than
T(n) <= O(hash) + O(N/m + 1) + T(n-1)
